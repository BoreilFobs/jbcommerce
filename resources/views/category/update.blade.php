...
<div class="main-content p-6 w-full h-full">
    <div class="card bg-white rounded-lg shadow-md p-8">
        <div class="flex items-center justify-between mb-6 border-b border-gray-200 pb-4">
            <h4 class="text-xl font-semibold text-gray-800">Update Category</h4>
        ...
        <form action="{{ url('/categories/' . $category->id . '/update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ $category->name }}" placeholder="Category Name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
            </div>

            <div>
                <label for="image_path" class="block text-sm font-medium text-gray-700 mb-1">Category Image</label>
                <input type="file" name="image" id="image_path" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            </div>
            
            <div id="image-preview-container" class="mt-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Image Preview</label>
                <img id="image-preview" src="{{ 'storage/' . $category->image_path}}" alt="Image Preview" class="w-48 h-48 object-cover rounded-lg border border-gray-300 shadow-sm">
            </div>
