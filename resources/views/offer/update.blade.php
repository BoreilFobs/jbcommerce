...
<div class="main-content p-4 sm:p-6 w-full h-full">
    <div class="card bg-white rounded-lg shadow-xl p-4 sm:p-8">
        <div class="flex flex-col sm:flex-row items-center justify-between mb-6 sm:mb-8 border-b border-gray-200 pb-4 gap-4">
            <h4 class="text-xl sm:text-2xl font-bold text-gray-800">Modifier le Produit</h4>
        ...
        <form action="{{ route('offer.update', $offer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $offer->name) }}" placeholder="Entrez le nom du produit" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                </div>
                <div>
                    <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                    <textarea name="description" id="description" rows="3" placeholder="Entrez la description du produit" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>{{ old('description', $offer->description) }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-4">
                <div>
                    ...
                    <select name="category" id="category" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', $offer->price) }}" placeholder="Entrez le prix" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                </div>
            </div>

            <div class="mt-4">
                <label for="quantity" class="block text-sm font-medium text-gray-700 mb-1">Quantité</label>
                <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $offer->quantity) }}" placeholder="Entrez la quantité" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
            </div>
            
            <div class="mt-6">
                <label for="images" class="block text-sm font-medium text-gray-700 mb-1">Mettre à jour les images (maximum 5)</label>
                    ...
                    @if ($offer_images && is_array($offer_images))
                        @foreach ($offer_images as $index => $img)
                            <div class="relative group">
                                <img src="{{'/storage/offer_img/product' . $offer->id . "/" . $offer_images[$index]}}" 
                                     alt="Image actuelle" 
                                     class="w-full aspect-square object-cover rounded-lg border border-gray-300 shadow-sm">
...
<script>
    ...
    document.querySelectorAll('#image-preview-list .fa-trash').forEach(button => {
        button.addEventListener('click', function(e) {
            const imageWrapper = e.target.closest('.group');
            if (confirm('Êtes-vous sûr de vouloir supprimer cette image ?')) {
                imageWrapper.remove();
                // Here you would typically also want to update a hidden input
                // to track which images should be deleted on the server
            }
        });
    });
</script>
